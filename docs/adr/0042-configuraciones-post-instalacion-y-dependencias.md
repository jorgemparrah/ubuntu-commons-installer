...
- **Bug real expuesto al implementar esta ADR:** `install_powerlevel10k.sh` purgaba el paquete `zsh` en su propio `uninstall_tool`, pese a que `zsh` es compartido con `install_oh_my_zsh.sh`. Antes de `depends_on`, esto pasaba inadvertido; con la dependencia explícita, un `reinstall` de Powerlevel10k (fallback mecánico `uninstall_tool` + `install_tool`) purgaba `zsh` y luego `install_tool` rechazaba porque el `check_status` de Oh My Zsh (que también requiere `command -v zsh`) dejaba de reportar `INSTALLED`, aunque su clon siguiera intacto. Corregido quitando la purga de `zsh` de `uninstall_tool` de Powerlevel10k — desinstalar el tema no debe desinstalar el shell compartido, eso es responsabilidad de `install_oh_my_zsh.sh`.
