...
| 11 (Fase 1) | Modernización de instaladores — infraestructura compartida | In Progress | 2026-07-19 | `scripts/lib/installer_cli.sh` (dispatcher de 6 verbos) y `scripts/lib/apt.sh` (helpers APT) creados; `install_cmatrix.sh` migrado como piloto ([ADR 0029](adr/0029-contrato-completo-de-instalador-referencia.md)). El resto de los instaladores sigue sin migrar, a propósito — se hará en grupos pequeños en fases posteriores |

Hitos 12 en adelante siguen `Blocked`/pendientes — ver `docs/ROADMAP.md` para el detalle y las dependencias entre ellos.
