...
Cubierto hoy por: `tests/test_terminal_apps_apt_simple_contract.sh` (I25), `tests/test_ghostty_installer.sh` (I26), `tests/test_curl_script_contract.sh` (I27), `tests/test_obs_studio_installer.sh` (I28), `tests/test_install_profile.sh` (I30) y `tests/test_list_info_commands.sh` (I31), incluidos en `tests/docker/run-all-tests.sh` y cada uno en su propio job de CI (`terminal-apps-apt-simple-contract`, `ghostty-installer`, `curl-script-contract`, `obs-studio-installer`, `install-profile`, `list-info-commands`).
