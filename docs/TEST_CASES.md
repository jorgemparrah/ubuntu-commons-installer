...
También se extendieron `tests/test_installer_cli.sh` (nuevo verbo opcional `configure`, Hito 17: se invoca cuando está definido; sin implementación propia se rechaza con código 3 citando ADR 0042, igual que `update`/`repair` con ADR 0029) y `tests/test_flameshot_installer.sh` (nuevo `configure_tool()` de Flameshot: rechaza si el paquete no está instalado o si `gsettings` no está disponible; agrega el atajo `PrintScreen` y respalda la lista previa de atajos de GNOME antes de tocarla; es idempotente si el atajo ya existe) — no suman IDs nuevos, extienden la cobertura de I11 y I16 respectivamente.

Cubierto hoy por: `tests/test_terminal_apps_apt_simple_contract.sh` (I25), `tests/test_ghostty_installer.sh` (I26), `tests/test_curl_script_contract.sh` (I27), `tests/test_obs_studio_installer.sh` (I28), `tests/test_install_profile.sh` (I30), `tests/test_list_info_commands.sh` (I31), `tests/test_dependencies_lib.sh` (I32), `tests/test_powerlevel10k_dependency.sh` (I33) y `tests/test_virtualbox_installer.sh` (I34), incluidos en `tests/docker/run-all-tests.sh` y cada uno en su propio job de CI (`terminal-apps-apt-simple-contract`, `ghostty-installer`, `curl-script-contract`, `obs-studio-installer`, `install-profile`, `list-info-commands`, `dependencies-lib`, `powerlevel10k-dependency`, `virtualbox-installer`).
