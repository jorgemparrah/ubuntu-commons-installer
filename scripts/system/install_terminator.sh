...
# fallback a Snap guardado con 'command -v snap'): sin diferencias
# funcionales relevantes más allá del nombre de paquete/binario.
# Terminator no tiene un archivo de configuración propio GESTIONADO POR
# ESTE PROYECTO (el usuario puede tener su propio `~/.config/terminator/`,
# pero este instalador nunca lo toca, ni antes ni después de esta
# migración) — mismo perfil de riesgo que cmatrix.
#
...
#               Nunca toca `~/.config/terminator/` del usuario: 'purge'
#               solo elimina archivos de configuración a nivel de
#               sistema (`/etc`), nunca el home.
...
#               de purge+autoremove, evitando un efecto más amplio del
#               necesario para un paquete sin estado propio a nivel de
#               sistema que perder.
...
UCI_TERMINATOR_SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
# shellcheck source=../lib/apt.sh
source "${UCI_TERMINATOR_SCRIPT_DIR}/../lib/apt.sh"
# shellcheck source=../lib/installer_cli.sh
source "${UCI_TERMINATOR_SCRIPT_DIR}/../lib/installer_cli.sh"
