#!/usr/bin/env bash
# tests/test_dust_installer.sh
#
# Prueba simulada (mocks) de scripts/system/install_dust.sh (Hito 39, ver
# docs/ROADMAP.md). Mecanismo deb-direct, URL resuelta dinámicamente
# contra la API de GitHub Releases (scripts/lib/github_release.sh),
# mismo patrón que LocalSend/Hoppscotch/Beekeeper Studio/DbGate/Lutris/
# Heroic. No instala nada real: apt-get/apt/dpkg/sudo/curl se
# interceptan con comandos falsos en un PATH temporal; 'curl' se usa
# SOLO para la API de GitHub, la descarga real del `.deb` la hace 'wget'
# (separado de 'curl'). El paquete se llama `du-dust` pero el binario es
# `dust` — el mock distingue ambos nombres a propósito.
#
# Uso:
#   bash tests/test_dust_installer.sh
set -Eeuo pipefail
...
INSTALL_SH="${UCI_REPO_ROOT}/scripts/system/install_dust.sh"
readonly INSTALL_SH
readonly UCI_PKG_NAME="du-dust"
readonly UCI_FAKE_DEB_URL="https://github.com/bootandy/dust/releases/download/v1.2.4/du-dust_1.2.4-1_amd64.deb"
...
if [[ "\$1" == "list" && "${upgradable}" == "yes" ]]; then
    echo "${UCI_PKG_NAME}/noble 1.2.5-1 amd64 [upgradable from: 1.2.4-1]"
fi
exit 0
EOF
...
echo '{"assets": [{"browser_download_url": "${UCI_FAKE_DEB_URL}"}]}'
exit 0
EOF
...
EOF
    ...
    if [[ "${create_binary}" == "yes" ]]; then
        cat > "${UCI_MOCK_BIN}/dust" <<'EOF'
#!/usr/bin/env bash
exit 0
EOF
        chmod +x "${UCI_MOCK_BIN}/dust"
    fi
}
...
if grep -q "curl.*api.github.com/repos/bootandy/dust/releases/latest" "${UCI_MOCK_LOG}"; then
...
if grep -qF "wget -O dust.deb ${UCI_FAKE_DEB_URL}" "${UCI_MOCK_LOG}"; then
...
if grep -q "apt-get install -y ./dust.deb" "${UCI_MOCK_LOG}"; then
