#!/usr/bin/env bash
# tests/test_onlyoffice_installer.sh
#
# Prueba simulada (mocks) de scripts/productivity/install_onlyoffice.sh
# (Hito 26, ver docs/ROADMAP.md). No instala nada real:
...
# rutas reales del sistema — sin mockearlos, un 'sudo' de passthrough
# directo terminaría llamando a los binarios reales del sistema y
# fallando por permisos en un contenedor no-root de CI (bug real ya
# encontrado y corregido en tests/test_virtualbox_installer.sh, mismo
# criterio aplicado acá desde el principio).
#
# Uso:
#   bash tests/test_onlyoffice_installer.sh
set -Eeuo pipefail
...
INSTALL_SH="${UCI_REPO_ROOT}/scripts/productivity/install_onlyoffice.sh"
readonly INSTALL_SH
readonly UCI_PKG_NAME="onlyoffice-desktopeditors"
...
if [[ "\$1" == "list" && "${upgradable}" == "yes" ]]; then
    echo "${UCI_PKG_NAME}/noble 8.0-1 amd64 [upgradable from: 7.0-1]"
fi
exit 0
EOF
...
EOF
    ...
    if [[ "${create_binary}" == "yes" ]]; then
        cat > "${UCI_MOCK_BIN}/onlyoffice-desktopeditors" <<'EOF'
#!/usr/bin/env bash
exit 0
EOF
        chmod +x "${UCI_MOCK_BIN}/onlyoffice-desktopeditors"
    fi
}
...
if grep -q "download.onlyoffice.com/GPG-KEY-ONLYOFFICE" "${UCI_MOCK_LOG}"; then
    pass "'install' descarga la clave GPG oficial de ONLYOFFICE"
else
    fail "'install' no descargó la clave esperada. Log: $(cat "${UCI_MOCK_LOG}")"
fi
if grep -q "install .*onlyoffice.gpg" "${UCI_MOCK_LOG}"; then
    pass "'install' escribe el keyring dearmored"
else
    fail "'install' no escribió el keyring esperado. Log: $(cat "${UCI_MOCK_LOG}")"
fi
if grep -q "tee /etc/apt/sources.list.d/onlyoffice.list" "${UCI_MOCK_LOG}"; then
    pass "'install' escribe /etc/apt/sources.list.d/onlyoffice.list"
else
