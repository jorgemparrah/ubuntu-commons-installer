#!/usr/bin/env bash
# tests/test_wireshark_installer.sh
#
# Prueba simulada (mocks) de scripts/system/install_wireshark.sh (Hito
# 52, ver docs/ROADMAP.md). No instala nada real ni toca grupos del
# sistema: apt-get/apt/dpkg/sudo/usermod/gpasswd/groups/
# debconf-set-selections se interceptan con comandos falsos en un PATH
# temporal.
#
# Wireshark no es un apt-simple más: su paquete `wireshark-common`
# pregunta vía debconf (`wireshark-common/install-setuid`, default
# `false`) si los usuarios no-root pueden capturar paquetes, y solo con
# `true` crea el grupo `wireshark` y aplica setcap sobre `dumpcap`. Este
# test verifica los dos pasos que el instalador agrega sobre el patrón
# apt-simple: el preseed de esa respuesta ANTES de instalar, y el alta
# del usuario en el grupo después (mismo patrón que `vboxusers` en
# tests/test_virtualbox_installer.sh).
#
# Uso:
#   bash tests/test_wireshark_installer.sh
set -Eeuo pipefail

UCI_TEST_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
readonly UCI_TEST_DIR
UCI_REPO_ROOT="$(cd "${UCI_TEST_DIR}/.." && pwd)"
readonly UCI_REPO_ROOT
INSTALL_SH="${UCI_REPO_ROOT}/scripts/system/install_wireshark.sh"
readonly INSTALL_SH
readonly UCI_PKG_NAME="wireshark"

# shellcheck source=lib/assertions.sh
source "${UCI_TEST_DIR}/lib/assertions.sh"

UCI_MOCK_BIN=""
UCI_MOCK_LOG=""

# setup_mock_bin <dpkg_state: ii|missing> [<upgradable: yes|no>] [<binary: auto|yes|no>] [<en_grupo: yes|no>]
setup_mock_bin() {
    local dpkg_state="$1" upgradable="${2:-no}" binary="${3:-auto}" en_grupo="${4:-yes}"
    UCI_MOCK_BIN="$(mktemp -d)"
    UCI_MOCK_LOG="$(mktemp)"

    cat > "${UCI_MOCK_BIN}/dpkg" <<EOF
#!/usr/bin/env bash
echo "dpkg \$*" >> "${UCI_MOCK_LOG}"
if [[ "\$1" == "-l" ]]; then
    if [[ "${dpkg_state}" == "ii" ]]; then
        echo "ii  ${UCI_PKG_NAME}  4.2.2  amd64  paquete de prueba"
        exit 0
    fi
    exit 1
fi
if [[ "\$1" == "--configure" ]]; then
    exit 0
fi
exit 0
EOF
    chmod +x "${UCI_MOCK_BIN}/dpkg"

    cat > "${UCI_MOCK_BIN}/apt-get" <<EOF
#!/usr/bin/env bash
echo "apt-get \$*" >> "${UCI_MOCK_LOG}"
exit 0
EOF
    chmod +x "${UCI_MOCK_BIN}/apt-get"

    cat > "${UCI_MOCK_BIN}/apt" <<EOF
#!/usr/bin/env bash
echo "apt \$*" >> "${UCI_MOCK_LOG}"
if [[ "\$1" == "list" && "${upgradable}" == "yes" ]]; then
    echo "${UCI_PKG_NAME}/noble 4.4.0 amd64 [upgradable from: 4.2.2]"
fi
exit 0
EOF
    chmod +x "${UCI_MOCK_BIN}/apt"

    # 'sudo' registra la invocación (para poder afirmar sobre
    # DEBIAN_FRONTEND) y luego emula el comportamiento real de sudo con
    # asignaciones de entorno previas al comando: 'sudo VAR=val cmd' es
    # sintaxis válida de sudo, pero un passthrough ingenuo con "$@" haría
    # que bash intente ejecutar 'VAR=val' como comando (código 127) —
    # las asignaciones solo se reconocen en tiempo de parseo, no tras la
    # expansión. Se despojan y exportan antes de ejecutar el resto, mismo
    # patrón que tests/test_split_installers_contract.sh (que cubre
    # install_ubuntu_restricted_extras.sh, el otro instalador del
    # catálogo que usa DEBIAN_FRONTEND).
    cat > "${UCI_MOCK_BIN}/sudo" <<EOF
#!/usr/bin/env bash
echo "sudo \$*" >> "${UCI_MOCK_LOG}"
while [[ "\$#" -gt 0 && "\$1" == *=* && "\$1" != -* ]]; do
    export "\$1"
    shift
done
"\$@"
EOF
    chmod +x "${UCI_MOCK_BIN}/sudo"
    cat > "${UCI_MOCK_BIN}/debconf-set-selections" <<EOF
#!/usr/bin/env bash
echo "debconf-set-selections \$*" >> "${UCI_MOCK_LOG}"
cat >> "${UCI_MOCK_LOG}"
exit 0
EOF
    chmod +x "${UCI_MOCK_BIN}/debconf-set-selections"

    for cmd in usermod gpasswd; do
        cat > "${UCI_MOCK_BIN}/${cmd}" <<EOF
#!/usr/bin/env bash
echo "${cmd} \$*" >> "${UCI_MOCK_LOG}"
exit 0
EOF
        chmod +x "${UCI_MOCK_BIN}/${cmd}"
    done

    cat > "${UCI_MOCK_BIN}/groups" <<EOF
#!/usr/bin/env bash
if [[ "${en_grupo}" == "yes" ]]; then
    echo "usuario wireshark sudo"
else
    echo "usuario sudo"
fi
EOF
    chmod +x "${UCI_MOCK_BIN}/groups"

    local create_binary="no"
    if [[ "${binary}" == "yes" ]]; then
        create_binary="yes"
    elif [[ "${binary}" == "auto" && "${dpkg_state}" == "ii" ]]; then
        create_binary="yes"
    fi
    if [[ "${create_binary}" == "yes" ]]; then
        cat > "${UCI_MOCK_BIN}/wireshark" <<'EOF'
#!/usr/bin/env bash
exit 0
EOF
        chmod +x "${UCI_MOCK_BIN}/wireshark"
    fi
}

teardown_mock_bin() {
    rm -rf "${UCI_MOCK_BIN}"
    rm -f "${UCI_MOCK_LOG}"
}

# run_installer <acción> <dpkg_state> [<upgradable>] [<binary>] [<en_grupo>]
RUN_CODE=0
RUN_OUTPUT=""
run_installer() {
    local action="$1" dpkg_state="$2" upgradable="${3:-no}" binary="${4:-auto}" en_grupo="${5:-yes}"
    setup_mock_bin "${dpkg_state}" "${upgradable}" "${binary}" "${en_grupo}"
    set +e
    RUN_OUTPUT="$(PATH="${UCI_MOCK_BIN}:${PATH}" bash "${INSTALL_SH}" "${action}" 2>&1)"
    RUN_CODE=$?
    set -e
}

echo "== 1. estado inicial ausente: NOT_INSTALLED =="
run_installer "status" "missing"
if [[ "${RUN_CODE}" -ne 0 ]] && [[ "${RUN_OUTPUT}" == *"NOT_INSTALLED"* ]]; then
    pass "estado inicial reporta NOT_INSTALLED con código distinto de cero"
else
    fail "estado inicial no reportó NOT_INSTALLED (código ${RUN_CODE}). Salida: ${RUN_OUTPUT}"
fi
teardown_mock_bin

echo ""
echo "== 2. install: preconfigura debconf, instala y agrega al grupo =="
run_installer "install" "missing"
if [[ "${RUN_CODE}" -eq 0 ]]; then
    pass "'install' sale con código 0"
else
    fail "'install' debería salir con código 0 (fue ${RUN_CODE}). Salida: ${RUN_OUTPUT}"
fi
if grep -q "wireshark-common/install-setuid boolean true" "${UCI_MOCK_LOG}"; then
    pass "'install' preconfigura 'install-setuid=true' vía debconf (sin esto el default es false y solo root captura)"
else
    fail "'install' no preconfiguró la respuesta debconf. Log: $(cat "${UCI_MOCK_LOG}")"
fi
# El preseed debe ocurrir ANTES de instalar el paquete: si se hiciera
# después, el postinst ya habría corrido con el default 'false'.
UCI_LINEA_DEBCONF="$(grep -n "install-setuid boolean true" "${UCI_MOCK_LOG}" | head -1 | cut -d: -f1)"
UCI_LINEA_INSTALL="$(grep -n "apt-get install -y ${UCI_PKG_NAME}" "${UCI_MOCK_LOG}" | head -1 | cut -d: -f1)"
if [[ -n "${UCI_LINEA_DEBCONF}" && -n "${UCI_LINEA_INSTALL}" && "${UCI_LINEA_DEBCONF}" -lt "${UCI_LINEA_INSTALL}" ]]; then
    pass "el preseed de debconf ocurre ANTES de instalar el paquete"
else
    fail "el preseed debería ocurrir antes del install (debconf: ${UCI_LINEA_DEBCONF:-ausente}, install: ${UCI_LINEA_INSTALL:-ausente})"
fi
if grep -q "DEBIAN_FRONTEND=noninteractive" "${UCI_MOCK_LOG}"; then
    pass "'install' usa DEBIAN_FRONTEND=noninteractive (apt no se cuelga esperando la pregunta)"
else
    fail "'install' no usó DEBIAN_FRONTEND=noninteractive. Log: $(cat "${UCI_MOCK_LOG}")"
fi
if grep -q "apt-get install -y ${UCI_PKG_NAME}" "${UCI_MOCK_LOG}"; then
    pass "'install' instala el paquete '${UCI_PKG_NAME}'"
else
    fail "'install' no instaló el paquete esperado. Log: $(cat "${UCI_MOCK_LOG}")"
fi
if grep -q "usermod -aG wireshark" "${UCI_MOCK_LOG}"; then
    pass "'install' agrega al usuario al grupo 'wireshark' (el paquete lo crea pero no agrega a nadie)"
else
    fail "'install' no agregó el usuario al grupo. Log: $(cat "${UCI_MOCK_LOG}")"
fi
if [[ "${RUN_OUTPUT}" == *"cerrar sesión"* ]]; then
    pass "'install' avisa que hay que reiniciar la sesión para que el grupo surta efecto"
else
    fail "'install' debería avisar sobre el reinicio de sesión. Salida: ${RUN_OUTPUT}"
fi
teardown_mock_bin

echo ""
echo "== 3. status con el paquete instalado: INSTALLED =="
run_installer "status" "ii"
if [[ "${RUN_CODE}" -eq 0 ]] && [[ "${RUN_OUTPUT}" == *"INSTALLED"* ]] && [[ "${RUN_OUTPUT}" != *"NOT_INSTALLED"* ]]; then
    pass "'status' reporta INSTALLED con código 0"
else
    fail "'status' no reportó INSTALLED correctamente (código ${RUN_CODE}). Salida: ${RUN_OUTPUT}"
fi
teardown_mock_bin

echo ""
echo "== 4. status con dpkg 'ii' pero sin binario resoluble: BROKEN =="
run_installer "status" "ii" "no" "no"
if [[ "${RUN_CODE}" -ne 0 ]] && [[ "${RUN_OUTPUT}" == *"BROKEN"* ]]; then
    pass "'status' reporta BROKEN si el binario 'wireshark' no resuelve"
else
    fail "'status' no reportó BROKEN correctamente (código ${RUN_CODE}). Salida: ${RUN_OUTPUT}"
fi
teardown_mock_bin

echo ""
echo "== 5. status con candidato de actualización: OUTDATED =="
run_installer "status" "ii" "yes"
if [[ "${RUN_CODE}" -eq 0 ]] && [[ "${RUN_OUTPUT}" == *"OUTDATED"* ]]; then
    pass "'status' reporta OUTDATED con candidato de actualización disponible"
else
    fail "'status' no reportó OUTDATED correctamente (código ${RUN_CODE}). Salida: ${RUN_OUTPUT}"
fi
teardown_mock_bin

echo ""
echo "== 6. install rechaza si está BROKEN (pide 'repair') =="
run_installer "install" "ii" "no" "no"
if [[ "${RUN_CODE}" -ne 0 ]] && [[ "${RUN_OUTPUT}" == *"repair"* ]]; then
    pass "'install' rechaza y sugiere 'repair' si está BROKEN"
else
    fail "'install' debería rechazar y sugerir 'repair' si está BROKEN (código ${RUN_CODE}). Salida: ${RUN_OUTPUT}"
fi
teardown_mock_bin

echo ""
echo "== 7. uninstall: purga el paquete y saca al usuario del grupo =="
run_installer "uninstall" "ii" "no" "auto" "yes"
if [[ "${RUN_CODE}" -eq 0 ]] && grep -q "apt-get purge -y ${UCI_PKG_NAME}" "${UCI_MOCK_LOG}"; then
    pass "'uninstall' purga el paquete (no 'remove')"
else
    fail "'uninstall' no purgó el paquete. Log: $(cat "${UCI_MOCK_LOG}")"
fi
if grep -q "gpasswd -d .* wireshark" "${UCI_MOCK_LOG}"; then
    pass "'uninstall' saca al usuario del grupo 'wireshark'"
else
    fail "'uninstall' no sacó al usuario del grupo. Log: $(cat "${UCI_MOCK_LOG}")"
fi
teardown_mock_bin

echo ""
echo "== 8. uninstall no falla si el usuario NO estaba en el grupo =="
run_installer "uninstall" "ii" "no" "auto" "no"
if [[ "${RUN_CODE}" -eq 0 ]]; then
    pass "'uninstall' sale con código 0 aunque el usuario no esté en el grupo"
else
    fail "'uninstall' debería salir con código 0 (fue ${RUN_CODE}). Salida: ${RUN_OUTPUT}"
fi
if grep -q "gpasswd -d" "${UCI_MOCK_LOG}"; then
    fail "'uninstall' no debería invocar gpasswd si el usuario no estaba en el grupo"
else
    pass "'uninstall' no invoca gpasswd si el usuario no estaba en el grupo"
fi
teardown_mock_bin

echo ""
echo "== 9. update invoca '--only-upgrade' =="
run_installer "update" "ii" "yes"
if [[ "${RUN_CODE}" -eq 0 ]] && grep -q "apt-get install --only-upgrade -y ${UCI_PKG_NAME}" "${UCI_MOCK_LOG}"; then
    pass "'update' invoca '--only-upgrade'"
else
    fail "'update' no se comportó como se esperaba (código ${RUN_CODE})"
fi
teardown_mock_bin

echo ""
echo "== 10. repair reaplica el preseed antes de reinstalar =="
run_installer "repair" "ii" "no" "no"
if [[ "${RUN_CODE}" -eq 0 ]] && grep -q "dpkg --configure -a" "${UCI_MOCK_LOG}" && grep -q "apt-get install --reinstall -y ${UCI_PKG_NAME}" "${UCI_MOCK_LOG}"; then
    pass "'repair' corre 'dpkg --configure -a' y reinstala el paquete"
else
    fail "'repair' no ejecutó los pasos esperados. Log: $(cat "${UCI_MOCK_LOG}")"
fi
if grep -q "wireshark-common/install-setuid boolean true" "${UCI_MOCK_LOG}"; then
    pass "'repair' reaplica el preseed de debconf (una instalación rota puede haber quedado con el default)"
else
    fail "'repair' no reaplicó el preseed. Log: $(cat "${UCI_MOCK_LOG}")"
fi
teardown_mock_bin

echo ""
echo "== 11. repair sobre NOT_INSTALLED se rechaza =="
run_installer "repair" "missing"
if [[ "${RUN_CODE}" -ne 0 ]] && [[ "${RUN_OUTPUT}" == *"install"* ]]; then
    pass "'repair' sobre NOT_INSTALLED rechaza y sugiere 'install'"
else
    fail "'repair' sobre NOT_INSTALLED no se comportó como se esperaba. Salida: ${RUN_OUTPUT}"
fi
teardown_mock_bin

echo ""
echo "== 12. reinstall usa '--reinstall' directo (sin purgar) =="
run_installer "reinstall" "ii"
if [[ "${RUN_CODE}" -eq 0 ]] && grep -q "apt-get install --reinstall -y ${UCI_PKG_NAME}" "${UCI_MOCK_LOG}"; then
    pass "'reinstall' invoca 'apt-get install --reinstall'"
else
    fail "'reinstall' no se comportó como se esperaba (código ${RUN_CODE}). Log: $(cat "${UCI_MOCK_LOG}")"
fi
if grep -q "purge" "${UCI_MOCK_LOG}"; then
    fail "'reinstall' no debería pasar por 'purge'"
else
    pass "'reinstall' evita el ciclo completo de purge+autoremove"
fi
teardown_mock_bin

echo ""
echo "== 13. subcomando inválido falla =="
run_installer "esto-no-existe" "missing"
if [[ "${RUN_CODE}" -ne 0 ]]; then
    pass "subcomando inválido sale con código distinto de cero"
else
    fail "subcomando inválido debería fallar"
fi
teardown_mock_bin

print_test_summary
exit_with_test_summary
