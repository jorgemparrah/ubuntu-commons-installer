#!/usr/bin/env bash
# tests/test_opentofu_installer.sh
#
# Prueba simulada (mocks) de scripts/development/install_opentofu.sh
# (Hito 42, ver docs/ROADMAP.md). No instala nada real:
# apt-get/apt/dpkg/sudo/curl/install/tee se interceptan con comandos
# falsos en un PATH temporal. La clave GPG de OpenTofu YA viene en
# formato binario (confirmado en vivo leyendo el script oficial de
# instalación, ver el propio instalador) — usa
# apt_vendor_repo_fetch_file_plain (como Brave), NUNCA 'gpg --dearmor'.
# La línea 'deb' del repositorio SÍ se construye a mano (distro fija
# "any", sin depender del codename), vía apt_vendor_repo_write_list.
#
# Uso:
#   bash tests/test_opentofu_installer.sh
set -Eeuo pipefail
...
INSTALL_SH="${UCI_REPO_ROOT}/scripts/development/install_opentofu.sh"
readonly INSTALL_SH
readonly UCI_PKG_NAME="tofu"
...
if [[ "\$1" == "list" && "${upgradable}" == "yes" ]]; then
    echo "${UCI_PKG_NAME}/noble 1.10.0 amd64 [upgradable from: 1.9.0]"
fi
exit 0
EOF
...
EOF
    ...
    if [[ "${create_binary}" == "yes" ]]; then
        cat > "${UCI_MOCK_BIN}/tofu" <<'EOF'
#!/usr/bin/env bash
exit 0
EOF
        chmod +x "${UCI_MOCK_BIN}/tofu"
    fi
}
...
if grep -q "tee /etc/apt/sources.list.d/opentofu.list" "${UCI_MOCK_LOG}"; then
...
if [[ "${RUN_CODE}" -ne 0 ]] && [[ "${RUN_OUTPUT}" == *"BROKEN"* ]]; then
    pass "'status' reporta BROKEN si el binario 'tofu' no resuelve"
else
