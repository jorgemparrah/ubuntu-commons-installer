#!/usr/bin/env bash
# tests/test_tailscale_installer.sh
#
# Prueba simulada (mocks) de scripts/system/install_tailscale.sh (Hito
# 46, ver docs/ROADMAP.md). No instala nada real:
# apt-get/apt/dpkg/sudo/curl/install/tee se interceptan con comandos
# falsos en un PATH temporal. La clave GPG de Tailscale YA viene en
# formato binario (confirmado en vivo con `curl` + `file`, igual que
# OpenTofu/Cloudflare Tunnel) — usa apt_vendor_repo_fetch_file_plain,
# NUNCA 'gpg --dearmor'. La línea 'deb' del repositorio SÍ depende del
# codename de la distro (simulado vía /etc/os-release real del
# contenedor de prueba, mismo criterio que install_virtualbox.sh).
#
# Uso:
#   bash tests/test_tailscale_installer.sh
set -Eeuo pipefail
...
INSTALL_SH="${UCI_REPO_ROOT}/scripts/system/install_tailscale.sh"
readonly INSTALL_SH
readonly UCI_PKG_NAME="tailscale"
...
if [[ "\$1" == "list" && "${upgradable}" == "yes" ]]; then
    echo "${UCI_PKG_NAME}/noble 1.80.0 amd64 [upgradable from: 1.78.0]"
fi
exit 0
EOF
...
EOF
    ...
    if [[ "${create_binary}" == "yes" ]]; then
        cat > "${UCI_MOCK_BIN}/tailscale" <<'EOF'
#!/usr/bin/env bash
exit 0
EOF
        chmod +x "${UCI_MOCK_BIN}/tailscale"
    fi
}
...
echo ""
echo "== 2. install: descarga la clave YA binaria (sin gpg --dearmor) y agrega el repo con codename dinámico =="
...
if grep -qE "^gpg " "${UCI_MOCK_LOG}"; then
    fail "'install' NO debería invocar 'gpg --dearmor' (la clave de Tailscale ya es binaria)"
else
    pass "'install' no invoca 'gpg --dearmor' (clave ya binaria)"
fi
if grep -q "install .*tailscale-archive-keyring.gpg" "${UCI_MOCK_LOG}"; then
    pass "'install' instala la clave binaria en su ruta final"
else
    fail "'install' no instaló la clave esperada. Log: $(cat "${UCI_MOCK_LOG}")"
fi
if grep -q "tee /etc/apt/sources.list.d/tailscale.list" "${UCI_MOCK_LOG}"; then
    pass "'install' escribe la línea 'deb' del repositorio con codename dinámico"
else
...
if [[ "${RUN_CODE}" -ne 0 ]] && [[ "${RUN_OUTPUT}" == *"BROKEN"* ]]; then
    pass "'status' reporta BROKEN si el binario 'tailscale' no resuelve"
else
