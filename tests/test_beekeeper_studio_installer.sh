#!/usr/bin/env bash
# tests/test_beekeeper_studio_installer.sh
#
# Prueba simulada (mocks) de
# scripts/development/install_beekeeper_studio.sh (Hito 32, ver
# docs/ROADMAP.md). Mecanismo deb-direct, URL resuelta dinámicamente
# contra la API de GitHub Releases (scripts/lib/github_release.sh), mismo
# patrón que LocalSend/Hoppscotch. No instala nada real:
...
# JSON falso) de la descarga real del `.deb` (hecha vía 'wget', separado
# de 'curl').
#
# Uso:
#   bash tests/test_beekeeper_studio_installer.sh
set -Eeuo pipefail
...
INSTALL_SH="${UCI_REPO_ROOT}/scripts/development/install_beekeeper_studio.sh"
readonly INSTALL_SH
readonly UCI_PKG_NAME="beekeeper-studio"
readonly UCI_FAKE_DEB_URL="https://github.com/beekeeper-studio/beekeeper-studio/releases/download/v5.9.2/beekeeper-studio_5.9.2_amd64.deb"
...
if [[ "\$1" == "list" && "${upgradable}" == "yes" ]]; then
    echo "${UCI_PKG_NAME}/noble 5.10.0-0 amd64 [upgradable from: 5.9.2-0]"
fi
exit 0
EOF
...
EOF
    ...
    if [[ "${create_binary}" == "yes" ]]; then
        cat > "${UCI_MOCK_BIN}/beekeeper-studio" <<'EOF'
#!/usr/bin/env bash
exit 0
EOF
        chmod +x "${UCI_MOCK_BIN}/beekeeper-studio"
    fi
}
...
if grep -q "curl.*api.github.com/repos/beekeeper-studio/beekeeper-studio/releases/latest" "${UCI_MOCK_LOG}"; then
...
if grep -qF "wget -O beekeeper-studio.deb ${UCI_FAKE_DEB_URL}" "${UCI_MOCK_LOG}"; then
...
if grep -q "apt-get install -y ./beekeeper-studio.deb" "${UCI_MOCK_LOG}"; then
