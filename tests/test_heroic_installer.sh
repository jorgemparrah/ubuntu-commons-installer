#!/usr/bin/env bash
# tests/test_heroic_installer.sh
#
# Prueba simulada (mocks) de scripts/productivity/install_heroic.sh
# (Hito 37, ver docs/ROADMAP.md). Mecanismo deb-direct, URL resuelta
# dinámicamente contra la API de GitHub Releases
# (scripts/lib/github_release.sh), mismo patrón que
# LocalSend/Hoppscotch/Beekeeper Studio/DbGate/Lutris. No instala nada
# real: apt-get/apt/dpkg/sudo/curl se interceptan con comandos falsos en
# un PATH temporal; 'curl' se usa SOLO para la API de GitHub, la descarga
# real del `.deb` la hace 'wget' (separado de 'curl').
#
# Uso:
#   bash tests/test_heroic_installer.sh
set -Eeuo pipefail
...
INSTALL_SH="${UCI_REPO_ROOT}/scripts/productivity/install_heroic.sh"
readonly INSTALL_SH
readonly UCI_PKG_NAME="heroic"
readonly UCI_FAKE_DEB_URL="https://github.com/Heroic-Games-Launcher/HeroicGamesLauncher/releases/download/v2.22.0/Heroic-2.22.0-linux-amd64.deb"
...
EOF
    ...
    if [[ "${create_binary}" == "yes" ]]; then
        cat > "${UCI_MOCK_BIN}/heroic" <<'EOF'
#!/usr/bin/env bash
exit 0
EOF
        chmod +x "${UCI_MOCK_BIN}/heroic"
    fi
}
...
if grep -q "curl.*api.github.com/repos/Heroic-Games-Launcher/HeroicGamesLauncher/releases/latest" "${UCI_MOCK_LOG}"; then
...
if grep -qF "wget -O heroic.deb ${UCI_FAKE_DEB_URL}" "${UCI_MOCK_LOG}"; then
...
if grep -q "apt-get install -y ./heroic.deb" "${UCI_MOCK_LOG}"; then
