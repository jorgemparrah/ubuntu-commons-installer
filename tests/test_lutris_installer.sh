#!/usr/bin/env bash
# tests/test_lutris_installer.sh
#
# Prueba simulada (mocks) de scripts/productivity/install_lutris.sh
...
# LocalSend/Hoppscotch/Beekeeper Studio/DbGate. No instala nada real:
# apt-get/apt/dpkg/sudo/curl se interceptan con comandos falsos en un
# PATH temporal; 'curl' se usa SOLO para la API de GitHub, la descarga
# real del `.deb` la hace 'wget' (separado de 'curl').
#
# Uso:
#   bash tests/test_lutris_installer.sh
set -Eeuo pipefail
...
INSTALL_SH="${UCI_REPO_ROOT}/scripts/productivity/install_lutris.sh"
readonly INSTALL_SH
readonly UCI_PKG_NAME="lutris"
readonly UCI_FAKE_DEB_URL="https://github.com/lutris/lutris/releases/download/v0.5.22/lutris_0.5.22_all.deb"
...
EOF
    ...
    if [[ "${create_binary}" == "yes" ]]; then
        cat > "${UCI_MOCK_BIN}/lutris" <<'EOF'
#!/usr/bin/env bash
exit 0
EOF
        chmod +x "${UCI_MOCK_BIN}/lutris"
    fi
}
...
if grep -q "curl.*api.github.com/repos/lutris/lutris/releases/latest" "${UCI_MOCK_LOG}"; then
...
if grep -qF "wget -O lutris.deb ${UCI_FAKE_DEB_URL}" "${UCI_MOCK_LOG}"; then
...
if grep -q "apt-get install -y ./lutris.deb" "${UCI_MOCK_LOG}"; then
