#!/usr/bin/env bash
# tests/test_signal_desktop_installer.sh
#
# Prueba simulada (mocks) de scripts/productivity/install_signal_desktop.sh
# (Hito 36, ver docs/ROADMAP.md). Primer instalador que combina AMBOS
# sub-mecanismos de apt_vendor_repo.sh: clave ASCII-armored que requiere
# 'gpg --dearmor' (mismo que VirtualBox/Slack) + archivo `.sources`
# completo en formato DEB822 ya armado (mismo que Brave/VSCodium/ngrok),
# sin construir una línea 'deb [...]' a mano. No instala nada real:
# apt-get/apt/dpkg/sudo/gpg/curl/install se interceptan con comandos
# falsos en un PATH temporal. 'curl' distingue la descarga de la clave
# (sin '-o', se pipea a 'gpg --dearmor') de la descarga del `.sources`
# (con '-o <destino>', vía apt_vendor_repo_fetch_file_plain). 'install' se
# mockea explícitamente (mismo criterio que
# tests/test_virtualbox_installer.sh/tests/test_slack_installer.sh): sin
...
#
# Uso:
#   bash tests/test_signal_desktop_installer.sh
set -Eeuo pipefail
...
INSTALL_SH="${UCI_REPO_ROOT}/scripts/productivity/install_signal_desktop.sh"
readonly INSTALL_SH
readonly UCI_PKG_NAME="signal-desktop"
...
if [[ "\$1" == "list" && "${upgradable}" == "yes" ]]; then
    echo "${UCI_PKG_NAME}/noble 7.30.0 amd64 [upgradable from: 7.29.0]"
fi
exit 0
EOF
...
EOF
    ...
    # 'curl' distingue: sin '-o' (pipeado a 'gpg --dearmor', descarga de
    # la clave armored) vs. con '-o <destino>' (apt_vendor_repo_fetch_file_plain,
    # descarga del .sources).
...
if [[ -n "\${dest}" ]]; then
    echo "contenido-falso-sources" > "\${dest}"
else
...
EOF
    ...
    if [[ "${create_binary}" == "yes" ]]; then
        cat > "${UCI_MOCK_BIN}/signal-desktop" <<'EOF'
#!/usr/bin/env bash
exit 0
EOF
        chmod +x "${UCI_MOCK_BIN}/signal-desktop"
    fi
}
...
echo ""
echo "== 2. install: dearmora la clave e instala el .sources ya armado =="
...
if grep -q "install .*signal-desktop-keyring.gpg" "${UCI_MOCK_LOG}"; then
    pass "'install' instala la clave dearmorada en su ruta final vía 'install -D'"
else
    fail "'install' no instaló la clave esperada. Log: $(cat "${UCI_MOCK_LOG}")"
fi
if grep -q "curl -fsSL https://updates.signal.org/static/desktop/apt/signal-desktop.sources" "${UCI_MOCK_LOG}"; then
    pass "'install' descarga el archivo .sources oficial ya armado"
else
    fail "'install' no descargó el .sources esperado. Log: $(cat "${UCI_MOCK_LOG}")"
fi
if grep -q "install .*signal-desktop.sources" "${UCI_MOCK_LOG}"; then
    pass "'install' instala el .sources en su ruta final vía 'install -D'"
else
    fail "'install' no instaló el .sources esperado. Log: $(cat "${UCI_MOCK_LOG}")"
fi
...
if [[ "${RUN_CODE}" -ne 0 ]] && [[ "${RUN_OUTPUT}" == *"BROKEN"* ]]; then
    pass "'status' reporta BROKEN si el binario 'signal-desktop' no resuelve"
else
