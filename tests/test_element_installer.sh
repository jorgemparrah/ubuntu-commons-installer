#!/usr/bin/env bash
# tests/test_element_installer.sh
#
# Prueba simulada (mocks) de scripts/productivity/install_element.sh
# (Hito 36, ver docs/ROADMAP.md). Mismo mecanismo que Brave/ngrok: clave
# ya lista para 'signed-by' (apt_vendor_repo_fetch_file_plain, sin 'gpg
# --dearmor') + una línea 'deb [...]' con distro fija ('default', no
# depende de la versión de Ubuntu, mismo patrón que Slack/OnlyOffice/
# ngrok, pero el mismo mecanismo apt_vendor_repo_write_list). No instala
# nada real: apt-get/apt/dpkg/sudo/curl/install/tee se interceptan con
# comandos falsos en un PATH temporal. 'install'/'tee' se mockean
# explícitamente (mismo criterio que tests/test_brave_installer.sh): sin
# mockearlos, 'sudo install -D'/'sudo tee' invocarían binarios reales del
# sistema contra rutas reales y fallarían por permisos en un contenedor
# no-root de CI.
#
# Uso:
#   bash tests/test_element_installer.sh
set -Eeuo pipefail
...
INSTALL_SH="${UCI_REPO_ROOT}/scripts/productivity/install_element.sh"
readonly INSTALL_SH
readonly UCI_PKG_NAME="element-desktop"
...
if [[ "\$1" == "list" && "${upgradable}" == "yes" ]]; then
    echo "${UCI_PKG_NAME}/noble 1.11.100-1 amd64 [upgradable from: 1.11.99-1]"
fi
exit 0
EOF
...
EOF
    ...
    if [[ "${create_binary}" == "yes" ]]; then
        cat > "${UCI_MOCK_BIN}/element-desktop" <<'EOF'
#!/usr/bin/env bash
exit 0
EOF
        chmod +x "${UCI_MOCK_BIN}/element-desktop"
    fi
}
...
echo ""
echo "== 2. install: descarga la clave, escribe el repo con distro fija 'default', instala =="
...
if grep -q "curl -fsSL https://packages.element.io/debian/element-io-archive-keyring.gpg" "${UCI_MOCK_LOG}"; then
    pass "'install' descarga la clave oficial de Element"
else
    fail "'install' no descargó la clave esperada. Log: $(cat "${UCI_MOCK_LOG}")"
fi
if grep -q "install .*element-io-archive-keyring.gpg" "${UCI_MOCK_LOG}"; then
    pass "'install' instala la clave en su ruta final vía 'install -D'"
else
    fail "'install' no instaló la clave esperada. Log: $(cat "${UCI_MOCK_LOG}")"
fi
if grep -q "tee /etc/apt/sources.list.d/element-io.list" "${UCI_MOCK_LOG}"; then
    pass "'install' escribe /etc/apt/sources.list.d/element-io.list"
else
...
if [[ "${RUN_CODE}" -ne 0 ]] && [[ "${RUN_OUTPUT}" == *"BROKEN"* ]]; then
    pass "'status' reporta BROKEN si el binario 'element-desktop' no resuelve"
else
