#!/usr/bin/env bash
# tests/test_steam_installer.sh
#
# Prueba simulada (mocks) de scripts/productivity/install_steam.sh (Hito
# 29, ver docs/ROADMAP.md). Mismo patrón apt-simple que install_ranger.sh,
# con una diferencia real: 'install_tool' habilita la arquitectura i386
# antes de instalar si todavía no está habilitada (requisito real de
# Steam, confirmado en la investigación de este hito). No instala nada
# real: apt-get/apt/dpkg/sudo se interceptan con comandos falsos en un
# PATH temporal.
#
# Uso:
#   bash tests/test_steam_installer.sh
set -Eeuo pipefail
...
INSTALL_SH="${UCI_REPO_ROOT}/scripts/productivity/install_steam.sh"
readonly INSTALL_SH
readonly UCI_PKG_NAME="steam-installer"
...
# setup_mock_bin <dpkg_state: ii|missing> [<upgradable: yes|no>] [<binary: auto|yes|no>] [<i386_ya_habilitada: yes|no>]
setup_mock_bin() {
    local dpkg_state="$1" upgradable="${2:-no}" binary="${3:-auto}" i386_habilitada="${4:-no}"
...
if [[ "\$1" == "list" && "${upgradable}" == "yes" ]]; then
    echo "${UCI_PKG_NAME}/noble 1.0.0.81-1 amd64 [upgradable from: 1.0.0.80-1]"
fi
exit 0
EOF
...
EOF
    ...
    if [[ "${create_binary}" == "yes" ]]; then
        cat > "${UCI_MOCK_BIN}/steam" <<'EOF'
#!/usr/bin/env bash
exit 0
EOF
        chmod +x "${UCI_MOCK_BIN}/steam"
    fi
}
...
# run_installer <acción> <dpkg_state> [<upgradable>] [<binary>] [<i386_ya_habilitada>]
RUN_CODE=0
RUN_OUTPUT=""
run_installer() {
    local action="$1" dpkg_state="$2" upgradable="${3:-no}" binary="${4:-auto}" i386_habilitada="${5:-no}"
    setup_mock_bin "${dpkg_state}" "${upgradable}" "${binary}" "${i386_habilitada}"
    set +e
...
echo ""
echo "== 2. install: habilita i386 si todavía no está habilitada =="
...
if grep -q "dpkg --add-architecture i386" "${UCI_MOCK_LOG}"; then
    pass "'install' habilita la arquitectura i386 antes de instalar"
else
    fail "'install' no habilitó i386. Log: $(cat "${UCI_MOCK_LOG}")"
fi
...
echo ""
echo "== 3. install: NO vuelve a habilitar i386 si ya estaba habilitada =="
run_installer "install" "missing" "no" "auto" "yes"
if [[ "${RUN_CODE}" -eq 0 ]] && ! grep -q "add-architecture" "${UCI_MOCK_LOG}"; then
    pass "'install' no vuelve a habilitar i386 si ya estaba habilitada"
else
...
if [[ "${RUN_CODE}" -ne 0 ]] && [[ "${RUN_OUTPUT}" == *"BROKEN"* ]]; then
    pass "'status' reporta BROKEN si el binario 'steam' no resuelve"
else
