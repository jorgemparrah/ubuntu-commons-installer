...
# (mismo criterio que tests/test_brave_installer.sh): sin mockearlos,
# 'sudo install -D'/'sudo tee' invocarían binarios reales del sistema
# contra rutas reales y fallarían por permisos en un contenedor no-root
# de CI.
#
# Uso:
#   bash tests/test_ngrok_installer.sh
set -Eeuo pipefail
...
INSTALL_SH="${UCI_REPO_ROOT}/scripts/development/install_ngrok.sh"
readonly INSTALL_SH
readonly UCI_PKG_NAME="ngrok"
...
if [[ "\$1" == "list" && "${upgradable}" == "yes" ]]; then
    echo "${UCI_PKG_NAME}/noble 3.20-1 amd64 [upgradable from: 3.19-1]"
fi
exit 0
EOF
...
EOF
    ...
    if [[ "${create_binary}" == "yes" ]]; then
        cat > "${UCI_MOCK_BIN}/ngrok" <<'EOF'
#!/usr/bin/env bash
exit 0
EOF
        chmod +x "${UCI_MOCK_BIN}/ngrok"
    fi
}
...
echo ""
echo "== 2. install: descarga la clave, escribe el repo con codename fijo 'bookworm', instala =="
...
if grep -q "curl -fsSL https://ngrok-agent.s3.amazonaws.com/ngrok.asc" "${UCI_MOCK_LOG}"; then
    pass "'install' descarga la clave oficial de ngrok"
else
    fail "'install' no descargó la clave esperada. Log: $(cat "${UCI_MOCK_LOG}")"
fi
if grep -q "install .*ngrok-archive-keyring.gpg" "${UCI_MOCK_LOG}"; then
...
if [[ "${RUN_CODE}" -ne 0 ]] && [[ "${RUN_OUTPUT}" == *"BROKEN"* ]]; then
    pass "'status' reporta BROKEN si el binario 'ngrok' no resuelve"
else
