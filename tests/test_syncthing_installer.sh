#!/usr/bin/env bash
# tests/test_syncthing_installer.sh
#
# Prueba simulada (mocks) de scripts/productivity/install_syncthing.sh
# (Hito 44, ver docs/ROADMAP.md). No instala nada real:
# apt-get/apt/dpkg/sudo/curl/install/tee se interceptan con comandos
# falsos en un PATH temporal. La clave GPG de Syncthing YA viene en
# formato binario (confirmado en vivo con `curl` + `file`, igual que
# OpenTofu) — usa apt_vendor_repo_fetch_file_plain, NUNCA
# 'gpg --dearmor'. La línea 'deb' del repositorio se construye a mano
# (distro fija "syncthing"/componente "stable-v2", sin depender del
# codename), vía apt_vendor_repo_write_list.
#
# Uso:
#   bash tests/test_syncthing_installer.sh
set -Eeuo pipefail
...
INSTALL_SH="${UCI_REPO_ROOT}/scripts/productivity/install_syncthing.sh"
readonly INSTALL_SH
readonly UCI_PKG_NAME="syncthing"
...
if [[ "\$1" == "list" && "${upgradable}" == "yes" ]]; then
    echo "${UCI_PKG_NAME}/noble 2.1.2 amd64 [upgradable from: 1.27.2]"
fi
exit 0
EOF
...
EOF
    ...
    if [[ "${create_binary}" == "yes" ]]; then
        cat > "${UCI_MOCK_BIN}/syncthing" <<'EOF'
#!/usr/bin/env bash
exit 0
EOF
        chmod +x "${UCI_MOCK_BIN}/syncthing"
    fi
}
...
echo ""
echo "== 2. install: descarga la clave YA binaria (sin gpg --dearmor) y agrega el repo =="
...
if grep -qE "^gpg " "${UCI_MOCK_LOG}"; then
    fail "'install' NO debería invocar 'gpg --dearmor' (la clave de Syncthing ya es binaria)"
else
    pass "'install' no invoca 'gpg --dearmor' (clave ya binaria)"
fi
if grep -q "install .*syncthing-archive-keyring.gpg" "${UCI_MOCK_LOG}"; then
    pass "'install' instala la clave binaria en su ruta final"
else
    fail "'install' no instaló la clave esperada. Log: $(cat "${UCI_MOCK_LOG}")"
fi
if grep -q "tee /etc/apt/sources.list.d/syncthing.list" "${UCI_MOCK_LOG}"; then
    pass "'install' escribe la línea 'deb' del repositorio (distro fija 'syncthing'/'stable-v2')"
else
...
if [[ "${RUN_CODE}" -ne 0 ]] && [[ "${RUN_OUTPUT}" == *"BROKEN"* ]]; then
    pass "'status' reporta BROKEN si el binario 'syncthing' no resuelve"
else
