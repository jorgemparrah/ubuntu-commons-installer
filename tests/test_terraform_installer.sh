#!/usr/bin/env bash
# tests/test_terraform_installer.sh
#
# Prueba simulada (mocks) de scripts/development/install_terraform.sh
...
#
# Uso:
#   bash tests/test_terraform_installer.sh
set -Eeuo pipefail
...
INSTALL_SH="${UCI_REPO_ROOT}/scripts/development/install_terraform.sh"
readonly INSTALL_SH
readonly UCI_PKG_NAME="terraform"
...
EOF
    ...
    if [[ "${create_binary}" == "yes" ]]; then
        cat > "${UCI_MOCK_BIN}/terraform" <<'EOF'
#!/usr/bin/env bash
exit 0
EOF
        chmod +x "${UCI_MOCK_BIN}/terraform"
    fi
}
...
if [[ "${RUN_CODE}" -ne 0 ]] && [[ "${RUN_OUTPUT}" == *"BROKEN"* ]]; then
    pass "'status' reporta BROKEN si el binario 'terraform' no resuelve"
else
