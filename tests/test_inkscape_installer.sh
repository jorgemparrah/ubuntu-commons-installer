#!/usr/bin/env bash
# tests/test_inkscape_installer.sh
#
# Prueba simulada (mocks) de scripts/system/install_inkscape.sh (Hito
# 35, ver docs/ROADMAP.md), mecanismo PPA igual que
# install_keepassxc.sh/install_ulauncher.sh: ppa:inkscape.dev/stable
# (oficial del propio equipo de Inkscape, confirmado activo en
# Launchpad). No instala nada real: apt-get/apt/dpkg/sudo/
# add-apt-repository se interceptan con comandos falsos en un PATH
# temporal.
#
# Uso:
#   bash tests/test_inkscape_installer.sh
set -Eeuo pipefail
...
INSTALL_SH="${UCI_REPO_ROOT}/scripts/system/install_inkscape.sh"
readonly INSTALL_SH
readonly UCI_PKG_NAME="inkscape"
readonly UCI_PPA="ppa:inkscape.dev/stable"
...
if [[ "\$1" == "list" && "${upgradable}" == "yes" ]]; then
    echo "${UCI_PKG_NAME}/noble 1.4.4-1 amd64 [upgradable from: 1.4.3-1]"
fi
exit 0
EOF
...
EOF
    ...
    if [[ "${create_binary}" == "yes" ]]; then
        cat > "${UCI_MOCK_BIN}/inkscape" <<'EOF'
#!/usr/bin/env bash
exit 0
EOF
        chmod +x "${UCI_MOCK_BIN}/inkscape"
    fi
}
...
if [[ "${RUN_CODE}" -ne 0 ]] && [[ "${RUN_OUTPUT}" == *"BROKEN"* ]]; then
    pass "'status' reporta BROKEN si el binario 'inkscape' no resuelve"
else
