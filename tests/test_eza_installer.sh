#!/usr/bin/env bash
# tests/test_eza_installer.sh
#
# Prueba simulada (mocks) de scripts/system/install_eza.sh (Hito 45, ver
# docs/ROADMAP.md). No instala nada real: apt-get/apt/dpkg/sudo/gpg/curl/
# install/tee se interceptan con comandos falsos en un PATH temporal.
# Mismo mecanismo que install_terraform.sh (clave dearmorada + línea
# 'deb' construida a mano), pero distro fija `stable`/`main` (sin
# depender del codename).
#
# Uso:
#   bash tests/test_eza_installer.sh
set -Eeuo pipefail
...
INSTALL_SH="${UCI_REPO_ROOT}/scripts/system/install_eza.sh"
readonly INSTALL_SH
readonly UCI_PKG_NAME="eza"
...
if [[ "\$1" == "list" && "${upgradable}" == "yes" ]]; then
    echo "${UCI_PKG_NAME}/noble 0.20.0 amd64 [upgradable from: 0.19.0]"
fi
exit 0
EOF
...
EOF
    ...
    if [[ "${create_binary}" == "yes" ]]; then
        cat > "${UCI_MOCK_BIN}/eza" <<'EOF'
#!/usr/bin/env bash
exit 0
EOF
        chmod +x "${UCI_MOCK_BIN}/eza"
    fi
}
...
echo ""
echo "== 2. install: dearmora la clave y agrega el repo con distro fija 'stable' =="
...
if grep -q "install .*gierens.gpg" "${UCI_MOCK_LOG}"; then
    pass "'install' instala la clave dearmorada en su ruta final"
else
    fail "'install' no instaló la clave esperada. Log: $(cat "${UCI_MOCK_LOG}")"
fi
if grep -q "tee /etc/apt/sources.list.d/gierens.list" "${UCI_MOCK_LOG}"; then
    pass "'install' escribe la línea 'deb' del repositorio (distro fija 'stable')"
else
...
if [[ "${RUN_CODE}" -ne 0 ]] && [[ "${RUN_OUTPUT}" == *"BROKEN"* ]]; then
    pass "'status' reporta BROKEN si el binario 'eza' no resuelve"
else
