#!/usr/bin/env bash
# tests/test_obs_studio_installer.sh
#
# Prueba simulada (mocks) de scripts/system/install_obs_studio.sh, migrado
# de Snap a su PPA oficial (ppa:obsproject/obs-studio, ver
# docs/adr/0038-obs-studio-migra-de-snap-a-ppa-oficial.md). Mismo patrón
# exacto que tests/test_ulauncher_installer.sh (el otro instalador del
# catálogo que agrega un PPA vía add-apt-repository). No instala nada
# real ni toca la red — no hay prueba funcional real equivalente a L01 en
# esta ronda (ver ADR 0038).
#
# Uso:
#   bash tests/test_obs_studio_installer.sh
set -Eeuo pipefail
...
INSTALL_SH="${UCI_REPO_ROOT}/scripts/system/install_obs_studio.sh"
readonly INSTALL_SH
readonly UCI_BIN_NAME="obs"
readonly UCI_PKG_NAME="obs-studio"
readonly UCI_PPA="ppa:obsproject/obs-studio"
...
if [[ "${RUN_CODE}" -eq 0 ]] && grep -q "add-apt-repository -y universe" "${UCI_MOCK_LOG}" && grep -q "add-apt-repository -y ${UCI_PPA}" "${UCI_MOCK_LOG}" && grep -q "apt-get install -y ${UCI_PKG_NAME}" "${UCI_MOCK_LOG}"; then
