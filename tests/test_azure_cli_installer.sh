#!/usr/bin/env bash
# tests/test_azure_cli_installer.sh
#
# Prueba simulada (mocks) de scripts/development/install_azure_cli.sh
# (Hito 42, ver docs/ROADMAP.md). No instala nada real:
# apt-get/apt/dpkg/sudo/gpg/curl/install/tee se interceptan con comandos
# falsos en un PATH temporal. Mismo mecanismo que
# install_terraform.sh (clave dearmorada + codename dinámico), pero el
# repositorio de Microsoft se escribe como un archivo `.sources` DEB822
# de varias líneas (Types/URIs/Suites/Components/Architectures/
# Signed-by), no una única línea 'deb [...]'.
#
# Uso:
#   bash tests/test_azure_cli_installer.sh
set -Eeuo pipefail
...
INSTALL_SH="${UCI_REPO_ROOT}/scripts/development/install_azure_cli.sh"
readonly INSTALL_SH
readonly UCI_PKG_NAME="azure-cli"
...
if [[ "\$1" == "list" && "${upgradable}" == "yes" ]]; then
    echo "${UCI_PKG_NAME}/noble 2.65.0 amd64 [upgradable from: 2.64.0]"
fi
exit 0
EOF
...
EOF
    ...
    if [[ "${create_binary}" == "yes" ]]; then
        cat > "${UCI_MOCK_BIN}/az" <<'EOF'
#!/usr/bin/env bash
exit 0
EOF
        chmod +x "${UCI_MOCK_BIN}/az"
    fi
}
...
echo ""
echo "== 2. install: dearmora la clave y agrega el repo .sources (DEB822) =="
...
if grep -q "install .*microsoft.gpg" "${UCI_MOCK_LOG}"; then
...
if grep -q "tee /etc/apt/sources.list.d/azure-cli.sources" "${UCI_MOCK_LOG}"; then
    pass "'install' escribe el archivo .sources del repositorio de Azure CLI"
else
...
if [[ "${RUN_CODE}" -ne 0 ]] && [[ "${RUN_OUTPUT}" == *"BROKEN"* ]]; then
    pass "'status' reporta BROKEN si el binario 'az' no resuelve"
else
