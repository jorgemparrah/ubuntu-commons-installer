#!/usr/bin/env bash
# tests/test_vagrant_installer.sh
#
# Prueba simulada (mocks) de scripts/development/install_vagrant.sh
# (Hito 48, ver docs/ROADMAP.md). No instala nada real:
# apt-get/apt/dpkg/sudo/gpg/curl/install/tee se interceptan con comandos
# falsos en un PATH temporal. Mismo criterio que
# tests/test_virtualbox_installer.sh: clave dearmorada + línea 'deb'
# construida a mano con codename dinámico (se simula vía /etc/os-release
# real del contenedor de prueba, sin mockear el propio archivo).
#
# Uso:
#   bash tests/test_vagrant_installer.sh
set -Eeuo pipefail
...
INSTALL_SH="${UCI_REPO_ROOT}/scripts/development/install_vagrant.sh"
readonly INSTALL_SH
readonly UCI_PKG_NAME="vagrant"
...
if [[ "\$1" == "list" && "${upgradable}" == "yes" ]]; then
    echo "${UCI_PKG_NAME}/noble 2.4.9-1 amd64 [upgradable from: 2.4.8-1]"
fi
exit 0
EOF
...
EOF
    ...
    if [[ "${create_binary}" == "yes" ]]; then
        cat > "${UCI_MOCK_BIN}/vagrant" <<'EOF'
#!/usr/bin/env bash
exit 0
EOF
        chmod +x "${UCI_MOCK_BIN}/vagrant"
    fi
}
...
echo ""
echo "== 2. install: dearmora la clave y agrega el repo con codename dinámico =="
...
if grep -q "install .*hashicorp-archive-keyring.gpg" "${UCI_MOCK_LOG}"; then
    pass "'install' instala la clave dearmorada en su ruta final"
else
    fail "'install' no instaló la clave esperada. Log: $(cat "${UCI_MOCK_LOG}")"
fi
if grep -q "tee /etc/apt/sources.list.d/hashicorp.list" "${UCI_MOCK_LOG}"; then
    pass "'install' escribe la línea 'deb' del repositorio de HashiCorp"
else
...
if [[ "${RUN_CODE}" -ne 0 ]] && [[ "${RUN_OUTPUT}" == *"BROKEN"* ]]; then
    pass "'status' reporta BROKEN si el binario 'vagrant' no resuelve"
else
