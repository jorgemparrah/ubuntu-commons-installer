#!/usr/bin/env bash
# tests/test_dbgate_installer.sh
#
# Prueba simulada (mocks) de scripts/development/install_dbgate.sh
# (Hito 32, ver docs/ROADMAP.md). Mecanismo deb-direct, URL resuelta
# dinámicamente contra la API de GitHub Releases
# (scripts/lib/github_release.sh), mismo patrón que
# LocalSend/Hoppscotch/Beekeeper Studio. No instala nada real:
# apt-get/apt/dpkg/sudo/curl se interceptan con comandos falsos en un
# PATH temporal; 'curl' distingue la llamada a la API de GitHub (devuelve
# JSON falso, incluyendo un asset "premium" de pago para confirmar que el
# patrón de resolución lo excluye) de la descarga real del `.deb` (hecha
# vía 'wget', separado de 'curl').
#
# Uso:
#   bash tests/test_dbgate_installer.sh
set -Eeuo pipefail
...
INSTALL_SH="${UCI_REPO_ROOT}/scripts/development/install_dbgate.sh"
readonly INSTALL_SH
readonly UCI_PKG_NAME="dbgate"
readonly UCI_FAKE_DEB_URL="https://github.com/dbgate/dbgate/releases/download/v7.2.3/dbgate-7.2.3-linux_amd64.deb"
readonly UCI_FAKE_PREMIUM_URL="https://github.com/dbgate/dbgate/releases/download/v7.2.3/dbgate-premium-7.2.3-linux_x86_64.AppImage"
readonly UCI_FAKE_LATEST_ALIAS_URL="https://github.com/dbgate/dbgate/releases/download/v7.2.3/dbgate-latest.deb"
...
if [[ "\$1" == "list" && "${upgradable}" == "yes" ]]; then
    echo "${UCI_PKG_NAME}/noble 7.3.0-0 amd64 [upgradable from: 7.2.3-0]"
fi
exit 0
EOF
...
EOF
    ...
    # 'curl' se usa SOLO para consultar la API de GitHub Releases — la
    # descarga real del .deb la hace 'deb_direct_download' vía 'wget'.
    # El JSON falso incluye, además del .deb de la edición community, un
    # asset "premium" (AppImage, de pago) y el alias "dbgate-latest.deb"
    # — ninguno de los dos debe matchear el patrón de resolución.
...
echo '{"assets": [{"browser_download_url": "${UCI_FAKE_LATEST_ALIAS_URL}"}, {"browser_download_url": "${UCI_FAKE_PREMIUM_URL}"}, {"browser_download_url": "${UCI_FAKE_DEB_URL}"}]}'
exit 0
EOF
...
EOF
    ...
    if [[ "${create_binary}" == "yes" ]]; then
        cat > "${UCI_MOCK_BIN}/dbgate" <<'EOF'
#!/usr/bin/env bash
exit 0
EOF
        chmod +x "${UCI_MOCK_BIN}/dbgate"
    fi
}
...
echo ""
echo "== 2. install: resuelve el .deb de la edición community, ignorando 'premium' y el alias 'latest' =="
...
if grep -q "curl.*api.github.com/repos/dbgate/dbgate/releases/latest" "${UCI_MOCK_LOG}"; then
...
if grep -qF "wget -O dbgate.deb ${UCI_FAKE_DEB_URL}" "${UCI_MOCK_LOG}"; then
    pass "'install' descarga (vía wget) el .deb de la edición community, no el premium ni el alias 'latest'"
else
    fail "'install' no descargó el .deb esperado. Log: $(cat "${UCI_MOCK_LOG}")"
fi
if grep -q "apt-get install -y ./dbgate.deb" "${UCI_MOCK_LOG}"; then
