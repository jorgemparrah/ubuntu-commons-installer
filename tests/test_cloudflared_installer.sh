#!/usr/bin/env bash
# tests/test_cloudflared_installer.sh
#
# Prueba simulada (mocks) de scripts/system/install_cloudflared.sh (Hito
# 46, ver docs/ROADMAP.md). No instala nada real:
# apt-get/apt/dpkg/sudo/curl/install/tee se interceptan con comandos
# falsos en un PATH temporal. La clave GPG de Cloudflare Tunnel YA viene
# en formato binario (confirmado en vivo con `curl` + `file`, igual que
# OpenTofu/Tailscale) — usa apt_vendor_repo_fetch_file_plain, NUNCA
# 'gpg --dearmor'. La línea 'deb' del repositorio usa distro fija `any`
# (recomendada explícitamente por la documentación oficial sobre las
# variantes por codename), sin depender de la versión de Ubuntu.
#
# Uso:
#   bash tests/test_cloudflared_installer.sh
set -Eeuo pipefail
...
INSTALL_SH="${UCI_REPO_ROOT}/scripts/system/install_cloudflared.sh"
readonly INSTALL_SH
readonly UCI_PKG_NAME="cloudflared"
...
if [[ "\$1" == "list" && "${upgradable}" == "yes" ]]; then
    echo "${UCI_PKG_NAME}/noble 2025.7.0 amd64 [upgradable from: 2025.6.0]"
fi
exit 0
EOF
...
EOF
    ...
    if [[ "${create_binary}" == "yes" ]]; then
        cat > "${UCI_MOCK_BIN}/cloudflared" <<'EOF'
#!/usr/bin/env bash
exit 0
EOF
        chmod +x "${UCI_MOCK_BIN}/cloudflared"
    fi
}
...
echo ""
echo "== 2. install: descarga la clave YA binaria (sin gpg --dearmor) y agrega el repo con distro fija 'any' =="
...
if grep -qE "^gpg " "${UCI_MOCK_LOG}"; then
    fail "'install' NO debería invocar 'gpg --dearmor' (la clave de Cloudflare ya es binaria)"
else
    pass "'install' no invoca 'gpg --dearmor' (clave ya binaria)"
fi
if grep -q "install .*cloudflare-main.gpg" "${UCI_MOCK_LOG}"; then
    pass "'install' instala la clave binaria en su ruta final"
else
    fail "'install' no instaló la clave esperada. Log: $(cat "${UCI_MOCK_LOG}")"
fi
if grep -q "tee /etc/apt/sources.list.d/cloudflared.list" "${UCI_MOCK_LOG}"; then
    pass "'install' escribe la línea 'deb' del repositorio (distro fija 'any')"
else
...
if [[ "${RUN_CODE}" -ne 0 ]] && [[ "${RUN_OUTPUT}" == *"BROKEN"* ]]; then
    pass "'status' reporta BROKEN si el binario 'cloudflared' no resuelve"
else
